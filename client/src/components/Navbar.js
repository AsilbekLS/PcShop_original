...
const Navbar = () => {
  ...
  return (
    <nav>
        <div className="logo">
    <button onClick={()=>navigate('/')} >EPSONSHOP</button>

        </div>

        <div className="pages">

            <button onClick={()=>navigate('/login') }>Bosh menyu</button>
              <button  onClick={()=>navigate('/add') }>Aloqa</button>
              <button onClick={()=>navigate('/basket')} >Basccet</button>
              <button onClick={()=>navigate('/Login')}>Royxatdan o`tish</button>
        </div>

<div className="search">

  <div className="input">
                  <input type="text" name="" id="" placeholder='Qanday maxsulot izlayapsiz?'/>
                 <svg className='svg' xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
